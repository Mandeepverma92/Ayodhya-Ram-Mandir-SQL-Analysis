...
SELECT * FROM Temple WHERE Name= 'Ram Mandir';


-- 2)List all construction phases for the Ram Mandir?
SELECT cp.PhaseName FROM ConstructionPhase cp
INNER JOIN Temple t ON cp.TempleID=t.TempleID
WHERE t.Name ='Ram Mandir';

-- 3)Find the total amount of donations received for the Ram Mandir?
SELECT SUM(Amount) AS 'Total Amount' FROM donations d
INNER JOIN Temple t ON d.TempleID=t.TempleID
WHERE t.Name='Ram Mandir';

-- 4)Get details about the architecture of the Ram Mandir?
SELECT a.* FROM Architecture a
INNER JOIN Temple t ON a.TempleID=t.TempleID
WHERE t.Name='Ram Mandir';

-- 5)Retrieve events associated with the Ram Mandir?
SELECT e.* FROM Events e 
INNER JOIN Templeevents te ON e.EventID=te.EventID
INNER JOIN Temple t ON te.TempleID=t.TempleID
WHERE t.Name='Ram Mandir';


-- 6)Find donors who contributed more than 50000 towards the Ram Mandir?
SELECT d.DonorName FROM donations d 
INNER JOIN Temple t ON d.TempleID=t.TempleID
WHERE t.Name='Ram Mandir' AND d.Amount>50000;
...
SELECT t.Name, COUNT(e.EventID) AS "Total Events"
FROM Events e
INNER JOIN Templeevents te ON e.EventId = te.EventId  -- Fix the typo here
INNER JOIN Temple t ON t.TempleID = te.TempleId
GROUP BY t.Name;
